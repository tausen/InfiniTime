...
Timer::Timer(Controllers::Timer& timerController, Controllers::MotorController& motorController)
  ...
  switch (timer.GetState()) {
    case Controllers::Timer::TimerState::Ringing:
...
void Timer::SetInterfaceRinging() {
...
void Timer::ToggleRunning() {
  if (timer.GetState() == Controllers::Timer::TimerState::Ringing) {
    motorController.StopRinging();
    Stop();
  } else if (timer.GetState() == Controllers::Timer::TimerState::Stopped) {
...
void Timer::SetRinging() {
  timer.Ring();
  SetCounters(std::chrono::seconds(0));
  SetInterfaceRinging();
}
